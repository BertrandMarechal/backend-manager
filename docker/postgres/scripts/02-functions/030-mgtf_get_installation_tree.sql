...
BEGIN
        ...
        CREATE TEMP TABLE temp_database_scripts (
                ...
                pk_dbv_id INTEGER,
                dbv_version_id TEXT,
                dbv_database_to_use TEXT,
                dbv_user_to_use TEXT,
                depending INTEGER,
                depending_version_id TEXT,
                depending_database_to_use TEXT,
                depending_user_to_use TEXT,
                dvf_file_name TEXT,
        ...
        WITH
                ...
                (
                        ...
                        select
                                ...
                                dbv.pk_dbv_id,
                                dbv.dbv_version_id,
                                COALESCE(dbv.dbv_database_to_use, 'dummy db'),
                                COALESCE(dbv.dbv_user_to_use, 'dummy user'),
                                dbv_depending_on.pk_dbv_id as depending,
                                dbv_depending_on.dbv_version_id as depending_version_id,
                                COALESCE(dbv_depending_on.dbv_database_to_use, 'dummy db') as depending_database_to_use,
                                COALESCE(dbv_depending_on.dbv_user_to_use, 'dummy user') as depending_user_to_use,
                        ...
                        AND (i_database is null OR i_database = dbv.dbv_database_to_use)
        ...
        WITH raw_data AS (
                select *
                FROM (
                        select rep_folder_name, dbv_version_id, dbv_database_to_use, dbv_user_to_use, dvf_file_name, dvf_position, max(db_position) db_position
                        from temp_database_scripts
                        group by rep_folder_name, dbv_version_id, dbv_database_to_use, dbv_user_to_use, dvf_file_name, dvf_position
                ) a
                ORDER BY db_position, rep_folder_name, dbv_version_id, dbv_database_to_use, dbv_user_to_use, dvf_position
        )
        SELECT 
                rep_folder_name,
                dbv_version_id,
                ...
                (
                        ...
                        AND rd1.dbv_database_to_use = rd2.dbv_database_to_use
                        AND rd1.dbv_user_to_use = rd2.dbv_user_to_use
                )
        FROM raw_data rd1
